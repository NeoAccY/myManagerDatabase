...
$response3 = '';
    ...
    if(!$result){
        echo "No se puedieron listar ,as columnas de la tabla: {$table}".mysqli_error($conexion);
        exit;
    }
    ...
    if(isset($_POST['Execute'])){
        ...
        $result = mysqli_query($conexion, $sql);
        if($result){
            $response1 =  '<h5>Update table con exito...</h5>';
            $response2 =  nl2br($sql).'<br><br>';
        }else{
            $response3= "<br>Update table error: Ingrese una consulta SQL valida ->".mysqli_error($conexion);
        }
    }
    if(isset($_POST['Execute'])){
        $bd = $_POST['data'];
        ...
        $result = mysqli_query($conexion, $sql);
        if($result){
            $response1 =  '<h5>Alter table aplicado exitosamente...</h5>';
            $response2 =  nl2br($sql).'<br><br>';
        }else{
            $response3= "<br>Create a table error: Ingrese una consulta SQL valida ->".mysqli_error($conexion);
        }
    }
...
<body>
    <form action="" method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>"
        ...
        <div class="title-cards">
            <h2 style="color:red">Update Table</h2>
        </div>
        <div class="container-card">
            ...
            <div class="card">
                ...
                <div class="contenido-card">
                    ...
                    <br><label for="">Update table: <?php echo $table ?></label>
